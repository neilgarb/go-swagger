...
func (sv schemaValidations) SetType(val string) {
	list := strings.Split(val, ",")
	sv.current.Type = spec.StringOrArray(list)
}
...
func (scp *schemaParser) createParser(nm string, schema, ps *spec.Schema, fld *ast.Field) *sectionedParser {
	...
	if ps.Ref.String() == "" {
		...
		sp.taggers = []tagParser{
			...
			newSingleLineTagParser("type", &setType{&spec.SimpleSchema{Type: string(schemeType)}, schemaValidations{ps}, rxf(rxTypeFmt, "")}),
		...
		itemsTaggers := func(items *spec.Schema, level int) []tagParser {
			...
			return []tagParser{
				...
				newSingleLineTagParser(fmt.Sprintf("items%dType", level), &setType{&spec.SimpleSchema{Type: string(schemeType)}, schemaValidations{items}, rxf(rxTypeFmt, itemsPrefix)}),
			}
		}
